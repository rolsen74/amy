...
S32 error;
	...
	IExec->DebugPrintF( "AmySDL : ROMInit 1\n" );

	NewlibBase = IExec->OpenLibrary( "newlib.library", 53 );

	if ( ! NewlibBase )
	{
		IExec->DebugPrintF( "AmySDL : Error opening newlib v53" );
		goto bailout;
	}
	...
	if ( ! INewlib )
	{
		IExec->DebugPrintF( "AmySDL : Error opening newlib interface" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmySDL : ROMInit 2\n" );
	...
	if ( ! libBase )
	{
		IExec->DebugPrintF( "AmySDL : Error creating Device" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmySDL : ROMInit 3\n" );
	...
	IExec->DebugPrintF( "AmySDL : Rock'n Roll\n" );
	error = FALSE;
