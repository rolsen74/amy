...
S32 error;
	...
	IExec->DebugPrintF( "AmyZLib : ROMInit 1\n" );

	NewlibBase = IExec->OpenLibrary( "newlib.library", 53 );

	if ( ! NewlibBase )
	{
		IExec->DebugPrintF( "AmyZLib : Error opening newlib v53" );
		goto bailout;
	}
	...
	if ( ! INewlib )
	{
		IExec->DebugPrintF( "AmyZLib : Error opening newlib interface" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyZLib : ROMInit 2\n" );
	...
	if ( ! libBase )
	{
		IExec->DebugPrintF( "AmyZLib : Error creating Device" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyZLib : ROMInit 3\n" );
	...
	IExec->DebugPrintF( "AmyZLib : Rock'n Roll\n" );
	error = FALSE;
