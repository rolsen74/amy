...
S32 error;
	...
	IExec->DebugPrintF( "AmyInput : ROMInit 1\n" );

	NewlibBase = IExec->OpenLibrary( "newlib.library", 53 );

	if ( ! NewlibBase )
	{
		IExec->DebugPrintF( "AmyInput : Error opening newlib v53" );
		goto bailout;
	}
	...
	if ( ! INewlib )
	{
		IExec->DebugPrintF( "AmyInput : Error opening newlib interface" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyInput : ROMInit 2\n" );
	...
	if ( ! libBase )
	{
		IExec->DebugPrintF( "AmyInput : Error creating Device" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyInput : ROMInit 3\n" );
	...
	IExec->DebugPrintF( "AmyInput : Rock'n Roll\n" );
	error = FALSE;
