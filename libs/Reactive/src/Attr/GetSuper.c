
/*
** SPDX-License-Identifier: BSD-2-Clause
** Copyright (c) 2011-2026 Rene W. Olsen
** Target OS: AmigaOS
*/

// --

#include "src/All.h"

// --

U32 AMY_FUNC _main_Attr_GetSuper( struct ReactivePrivIFace *Self, struct RA_PrivObject *obj, U32 AttrID, PTR Storage )
{
struct RAP_Get msg;
U32 retval;

	if (( ! obj ) || ( obj->RA_Object_ID != ID_RAObject ))
	{
//		IExec->DebugPrintF( "Attr_GetSuper: Invalid Object Pointer (%p)", obj );
//		  Self->Error_LogTags( txt, obj );

		retval = RAV_General_InvalidObject;
		goto bailout;
	}

	if ( ! Storage )
	{
//		IExec->DebugPrintF( "Attr_GetSuper: Missing Storage Pointer" );
//		  Self->Error_LogTags( txt );

		retval = RAV_General_NoStorage;
		goto bailout;
	}

	msg.MethodID= RAM_Get;
	msg.AttrID	= AttrID;
	msg.Storage	= Storage;

	retval = Self->Method_DoSuperList( obj->RA_Object_Class, obj, (PTR) & msg );

bailout:

	return(	retval );
}
