...
IUP_SDK_API Iarray *iupArrayCreate( struct libData *data, int start_count, int elem_size )
{
...
IUP_SDK_API void iupArrayDestroy( struct libData *data, Iarray *iarray )
{
...
IUP_SDK_API void *iupArrayGetData( struct libData *data, Iarray *iarray )
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return NULL;
...
IUP_SDK_API void *iupArrayReleaseData( struct libData *data, Iarray *iarray )
{
  void *tmpdata;
  iupASSERT(iarray != NULL );
  if ( ! iarray )
...
IUP_SDK_API void *iupArrayInc( struct libData *data, Iarray *iarray )
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return NULL;
...
IUP_SDK_API void *iupArrayAdd( struct libData *data, Iarray *iarray, int add_count)
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return NULL;
...
IUP_SDK_API void *iupArrayInsert( struct libData *data, Iarray *iarray, int index, int insert_count)
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return NULL;
  if (index < 0 || index > iarray->count)
    return NULL;
  iupArrayAdd( data, iarray, insert_count );
...
IUP_SDK_API void iupArrayRemove( struct libData *data, Iarray *iarray, int index, int remove_count)
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return;
...
IUP_SDK_API int iupArrayCount( struct libData *data, Iarray *iarray )
{
  iupASSERT(iarray!=NULL );
  if ( ! iarray )
    return 0;
