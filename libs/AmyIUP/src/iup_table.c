...
static void iTableFreeItemArray(Itable_IndexTypes indexType, unsigned int nextFreeIndex, 
                                                            ItableItem *items );
static unsigned int iTableGetEntryIndex( struct libData *data, Itable *it, const char *key, unsigned long *keyIndex );
static unsigned int iTableFindItem( struct libData *data, Itable *it, const char *key, ItableEntry **entry, unsigned int *itemIndex, unsigned long *keyIndex );
static unsigned int iTableResize( struct libData *data, Itable *it );
static void iTableAdd( struct libData *data, Itable *it, ItableKey *key, void *value, Itable_Types itemType );
static void iTableUpdateArraySize( ItableEntry *entry );   

#ifdef DEBUGTABLE
static void iTableShowStatistics( struct libData *data, Itable *it );
...
IUP_SDK_API void iupTableClear( struct libData *data, Itable *it)
{
  unsigned int i;
...
IUP_SDK_API void iupTableDestroy( struct libData *data, Itable *it )
{
  if ( ! it)
    return;
...
IUP_SDK_API int iupTableCount( struct libData *data, Itable *it)
{
  iupASSERT(it!=NULL );
  if ( ! it)
    return 0;
...
IUP_SDK_API void iupTableSetFunc( struct libData *data, Itable *it, const char *key, Ifunc func)
{
	iupTableSet( data, it, key, (void*)func, IUPTABLE_FUNCPOINTER ); /* type cast from function pointer to void **/
}

IUP_SDK_API void iupTableSet( struct libData *data, Itable *it, const char *key, void *value, Itable_Types itemType )
{
	unsigned int  itemIndex, itemFound;
	unsigned long keyIndex;
	ItableEntry  *entry;
	ItableItem   *item;
	void         *v;
	...
	itemFound = iTableFindItem( data, it, key, &entry, &itemIndex, &keyIndex );
...
static void iTableRemoveItem( struct libData *data, Itable *it, ItableEntry *entry, unsigned int itemIndex)
{
	ItableItem   *item;
	unsigned int  i;
...
IUP_SDK_API void iupTableRemove( struct libData *data, Itable *it, const char *key )
{
	unsigned int itemFound,itemIndex;
	unsigned long keyIndex;
	ItableEntry  *entry;
	...
	if ( itemFound )
		iTableRemoveItem( data, it, entry, itemIndex );
}

IUP_SDK_API void *iupTableGet( struct libData *data, Itable *it, const char *key )
{
	unsigned int  itemFound,itemIndex;
	unsigned long keyIndex;
	ItableEntry  *entry;
...
IUP_SDK_API Ifunc iupTableGetFunc( struct libData *data, Itable *it, const char *key, void **value)
{
  Itable_Types itemType = IUPTABLE_POINTER;
  *value = iupTableGetTyped( data, it, key, &itemType );
...
IUP_SDK_API void *iupTableGetTyped( struct libData *data, Itable *it, const char *key, Itable_Types *itemType)
{
...
IUP_SDK_API void iupTableSetCurr( struct libData *data, Itable *it, void *value, Itable_Types itemType)
{
  void *v;
  ItableItem *item;
...
IUP_SDK_API void *iupTableGetCurr( struct libData *data, Itable *it)
{
  iupASSERT(it!=NULL );
  if ( ! it || it->context.entryIndex == (unsigned int)-1
         || it->context.itemIndex == (unsigned int)-1)
...
IUP_SDK_API int iupTableGetCurrType( struct libData *data, Itable *it)
{
  iupASSERT(it!=NULL );
  if ( ! it || it->context.entryIndex == (unsigned int)-1
         || it->context.itemIndex == (unsigned int)-1)
...
IUP_SDK_API char *iupTableFirst( struct libData *data, Itable *it)
{
...
IUP_SDK_API char *iupTableNext( struct libData *data, Itable *it)
{
  iupASSERT(it!=NULL );
  if ( ! it || it->context.entryIndex == (unsigned int)-1
         || it->context.itemIndex == (unsigned int)-1)
...
IUP_SDK_API char *iupTableRemoveCurr( struct libData *data, Itable *it)
{
  char *key;
  ItableEntry  *entry;
...
static unsigned int iTableFindItem( struct libData *data, Itable *it, const char *key, ItableEntry **entry, unsigned int *itemIndex, unsigned long *keyIndex)
{
	...
	entryIndex = iTableGetEntryIndex( data, it, key, keyIndex );
...
static void iTableAdd( struct libData *data, Itable *it, ItableKey *key, void *value, Itable_Types itemType )
{
	...
	entryIndex = iTableGetEntryIndex( data, it, key->keyStr, &keyIndex );
...
static unsigned int iTableResize( struct libData *data, Itable *it )
{
	unsigned int   newSizeIndex,entryIndex,i;
	Itable        *newTable;
	ItableItem    *item;
	...
	*/
	...
	newTable = iupTableCreateSized( data, it->indexType, newSizeIndex );
	...
	for (entryIndex = 0; entryIndex < it->entriesSize; entryIndex++)
	{
		...
		if (entry->items)
		{
			...
			for (i = 0; i < entry->nextItemIndex; i++, item++)
			{
				iTableAdd( data, newTable, &(item->key), item->value, item->itemType );
			}
