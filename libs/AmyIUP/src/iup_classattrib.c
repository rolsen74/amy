...
/* '*' is used in amy_IupMatrix to indicate a full line or column
...
static char *iClassGetDefaultValue( struct libData *data, IattribFunc *afunc )
{
  if (afunc->call_global_default)
    return amy_IupGetGlobal( data, afunc->default_value );
  else
    return (char*)afunc->default_value;
}

int iupClassObjectSetAttributeId2( struct libData *data, Ihandle *ih, const char *name, int id1, int id2, const char *value)
{
  IattribFunc* afunc;
  ...
  if (name[0]==0)
    name = "IDVALUE";  /* pure numbers are used as attributes in amy_IupList and amy_IupMatrix, 
                          translate them into IDVALUE. */
  afunc = (IattribFunc*) iupTableGet( data, ih->iclass->attrib_func, name );
  if (afunc)
  {         
        ...
        (ih->handle || afunc->flags & IUPAF_NOT_MAPPED))
    {
      ...
      if (afunc->flags & IUPAF_HAS_ID2)
      {
        ...
        return id2_set( data, ih, id1, id2, value );
      }
      else if (afunc->flags & IUPAF_HAS_ID)
      {
        IattribSetIdFunc id_set = (IattribSetIdFunc)afunc->set;
        return id_set( data, ih, id1, value );  /* id2 is ignored */
      }
    }
...
int iupClassObjectSetAttributeId( struct libData *data, Ihandle *ih, const char *name, int id, const char * value)
{
  IattribFunc* afunc;
  ...
  {         
        ...
        (ih->handle || afunc->flags & IUPAF_NOT_MAPPED))
    {
      /* id numbered attributes have default value NULL always */
      IattribSetIdFunc id_set = (IattribSetIdFunc)afunc->set;
      return id_set( data, ih, id, value );
    }
...
int iupClassObjectSetAttribute( struct libData *data, Ihandle *ih, const char *name, const char * value, int *inherit)
{
  IattribFunc* afunc;
  ...
  if (ih->iclass->has_attrib_id!=0)
  {
    ...
    if (name_id)
    {
      ...
      if ( ! partial_name)
        partial_name = "IDVALUE";  /* pure numbers are used as attributes in amy_IupList and amy_IupMatrix, 
                                      translate them into IDVALUE. */
      afunc = (IattribFunc*) iupTableGet( data, ih->iclass->attrib_func, partial_name );
      if (afunc && afunc->flags & IUPAF_HAS_ID)
      {         
    ...
    if (afunc->set && (ih->handle || afunc->flags & IUPAF_NOT_MAPPED))
    {
      ...
      if ( ! value)
      {
        ...
        if (*inherit && ih->parent)   
          value = iupAttribGetInherit( data, ih->parent, name ); 

        if ( ! value)
          value = iClassGetDefaultValue( data, afunc );
      }

      if (afunc->flags & IUPAF_HAS_ID2)
      {
        IattribSetId2Func id2_set = (IattribSetId2Func)afunc->set;
        return id2_set( data, ih, IUP_INVALID_ID, IUP_INVALID_ID, value );  /* empty Id */
      }
      else if (afunc->flags & IUPAF_HAS_ID)
      {
        IattribSetIdFunc id_set = (IattribSetIdFunc)afunc->set;
        return id_set( data, ih, IUP_INVALID_ID, value );  /* empty Id */
      }
      else
        ret = afunc->set( data, ih, value );
...
char *iupClassObjectGetAttributeId2( struct libData *data, Ihandle *ih, const char *name, int id1, int id2)
{
  IattribFunc* afunc;
  ...
  if (afunc)
  {
        ...
        (ih->handle || afunc->flags & IUPAF_NOT_MAPPED))
    {
      if (afunc->flags & IUPAF_HAS_ID2)
      {
        ...
        return id2_get( data, ih, id1, id2 );
      }
      else if (afunc->flags & IUPAF_HAS_ID)
      {
        IattribGetIdFunc id_get = (IattribGetIdFunc)afunc->get;
        return id_get( data, ih, id1 );  /* id2 is ignored */
      }
    }
  }
...
char *iupClassObjectGetAttributeId( struct libData *data, Ihandle *ih, const char *name, int id)
{
  IattribFunc* afunc;
...
char *iupClassObjectGetAttribute( struct libData *data, Ihandle *ih, const char *name, char **def_value, int *inherit)
{
  IattribFunc* afunc;
  ...
  *inherit = 1; /* default is inheritable */
  if (afunc)
  {
    *def_value = iClassGetDefaultValue( data, afunc );
    *inherit = !(afunc->flags & IUPAF_NO_INHERIT) &&   /* is inheritable */
               !(afunc->flags & IUPAF_NO_STRING );      /* is a string */
    ...
    if (afunc->get && (ih->handle || afunc->flags & IUPAF_NOT_MAPPED))
    {
      ...
      {
        IattribGetId2Func id2_get = (IattribGetId2Func)afunc->get;
        return id2_get( data, ih, IUP_INVALID_ID, IUP_INVALID_ID );  /* empty Id */
      }
      else if (afunc->flags & IUPAF_HAS_ID)
      {
        IattribGetIdFunc id_get = (IattribGetIdFunc)afunc->get;
        return id_get( data, ih, IUP_INVALID_ID );  /* empty Id */
      }
      else
        return afunc->get( data, ih );
    }
  }
  return NULL;
}

void iupClassObjectGetAttributeInfo( struct libData *data, Ihandle *ih, const char *name, char **def_value, int *inherit)
{
  IattribFunc* afunc;
...
void iupClassGetAttribNameInfo( struct libData *data, Iclass *ic, const char *name, char **def_value, int *flags)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
  *def_value = NULL;
  *flags = 0;
  if (afunc)
  {
...
int iupClassObjectCurAttribIsInherit( struct libData *data, Iclass *ic)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGetCurr( data, ic->attrib_func );
  if (afunc && !(afunc->flags & IUPAF_NO_INHERIT))
    return 1;
  return 0;
}

int iupClassObjectAttribCanCopy( struct libData *data, Ihandle *ih, const char *name)
{
  Iclass *ic = ih->iclass;
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
...
int iupClassObjectAttribIsNotString( struct libData *data, Ihandle *ih, const char *name)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ih->iclass->attrib_func, name );
  if (afunc && (afunc->flags & IUPAF_NO_STRING || afunc->flags & IUPAF_CALLBACK || afunc->flags & IUPAF_IHANDLE))
    return 1;
  return 0;
}

int iupClassObjectAttribIsIhandle( struct libData *data, Ihandle *ih, const char *name)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ih->iclass->attrib_func, name );
...
IUP_SDK_API int iupClassObjectAttribIsCallback( struct libData *data, Ihandle *ih, const char *name)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ih->iclass->attrib_func, name );
...
int iupClassAttribIsRegistered( struct libData *data, Iclass *ic, const char *name)
{
  ...
  if (ic->has_attrib_id!=0)
  {
    ...
    if (name_id)
    {
      ...
      if ( ! partial_name)
        partial_name = "IDVALUE";  /* pure numbers are used as attributes in amy_IupList and amy_IupMatrix, 
                                      translate them into IDVALUE. */
      afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, partial_name );
    }
  }

  if ( ! afunc)
    afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
...
static void iClassCheckAttrib( struct libData *data, Iclass *ic, const char *name, IattribGetFunc get, IattribSetFunc set, int flags)
{
...
IUP_SDK_API void iupClassRegisterAttribute( struct libData *data, Iclass *ic, const char *name,
                               ...
                               const char *_default_value, const char *_system_default, int _flags)
{
...
#ifdef IUP_ASSERT
  iClassCheckAttrib( data, ic, name, _get, _set, _flags );
#endif
  ...
  iupTableSet( data, ic->attrib_func, name, (void*)afunc, IUPTABLE_POINTER );
}

IUP_SDK_API void iupClassRegisterAttributeId( struct libData *data, Iclass *ic, const char *name,
...
#ifdef IUP_ASSERT
  iClassCheckAttrib( data, ic, name, (IattribGetFunc)_get, (IattribSetFunc)_set, _flags );
#endif
...
IUP_SDK_API void iupClassRegisterAttributeId2( struct libData *data, Iclass *ic, const char *name,
...
IUP_SDK_API void iupClassRegisterGetAttribute( struct libData *data, Iclass *ic, const char *name,
                                  IattribGetFunc *_get, IattribSetFunc *_set, 
                                  const char **_default_value, const char **_system_default, int *_flags)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
...
IUP_SDK_API void iupClassRegisterReplaceAttribFunc( struct libData *data, Iclass *ic, const char *name, IattribGetFunc _get, IattribSetFunc _set)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
  if (afunc)
  {
...
IUP_SDK_API void iupClassRegisterReplaceAttribDef( struct libData *data, Iclass *ic, const char *name, const char *_default_value, const char *_system_default)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
  if (afunc)
  {
...
IUP_SDK_API void iupClassRegisterReplaceAttribFlags( struct libData *data, Iclass *ic, const char *name, int _flags)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
  if (afunc)
    afunc->flags = _flags;
...
IUP_SDK_API void iupClassRegisterCallback( struct libData *data, Iclass *ic, const char *name, const char *format)
{
  ...
  iupClassRegisterAttribute( data, ic, name, NULL, NULL, format, NULL, IUPAF_NO_INHERIT|IUPAF_CALLBACK );
}

IUP_SDK_API char *iupClassCallbackGetFormat( struct libData *data, Iclass *ic, const char *name)
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, name );
  if (afunc)
    return (char*)afunc->default_value;
...
IUP_API int amy_IupGetClassAttributes( struct libData *data, const char *classname, char** names, int n)
{
  ...
  if ( ! names || n==0 || n==-1)
    return iupTableCount( data, ic->attrib_func );

  name = iupTableFirst( data, ic->attrib_func );
  while (name)
  {
    afunc = (IattribFunc*) iupTableGetCurr( data, ic->attrib_func );
...
IUP_API void amy_IupSetClassDefaultAttribute( struct libData *data, const char *classname, const char *name, const char *default_value)
{
  Iclass *ic;
  IattribFunc* afunc;
               ...
               !(afunc->flags & IUPAF_HAS_ID))
  {
    ...
    iupClassRegisterAttribute( data, ic, name, NULL, NULL, default_value, NULL, IUPAF_DEFAULT );
}
...
IUP_API void amy_IupSaveClassAttributes( struct libData *data, Ihandle *ih )
{
  ...
  if (iupClassMatch( data, ic, "tree" ) || /* tree can only set id attributes after map, so they can not be saved */
      iupClassMatch( data, ic, "cells" )) /* cells does not have any saveable id attributes */
    has_attrib_id = 0;  

  if (iupClassMatch( data, ic, "list" ))
    start_id = 1;
  ...
  while (name)
  {
    ...
    if (afunc && !(afunc->flags & IUPAF_NO_STRING) &&  /* is a string */
                 ...
                 !(afunc->flags & IUPAF_CALLBACK))     /* not a callback */
    {
      if ((afunc->flags&IUPAF_NO_SAVE) && iupBaseNoSaveCheck( data, ih, name))  /* can not be saved */
      {
        name = iupTableNext( data, ic->attrib_func );
        continue;
      }
      ...
      if ( ! (afunc->flags & IUPAF_HAS_ID))     /* no ID */
      {
        int inherit;
        char *def_value;
        char *value = iupClassObjectGetAttribute( data, ih, name, &def_value, &inherit );
        if (value && value[0] )    /* NOT NULL and not empty */
        {
          ...
          if ( ! iupStrEqualNoCase(value, iupAttribGet( data, ih, name)))     /* NOT already stored */
      ...
      else if (has_attrib_id)
      {
        ...
        if (afunc->flags&IUPAF_HAS_ID2)
        {
          ...
          for (lin=0; lin<numlin; lin++)
          {
            for (col=0; col<numcol; col++)
            {
              ...
              if (value && value[0] )  /* NOT NULL and not empty */
              {
                if ( ! iupStrEqualNoCase(value, iupAttribGetId2( data, ih, name, lin, col)))     /* NOT already stored */
        ...
        else
        {
          ...
          for (id=start_id; id<count+start_id; id++)
          {
            ...
            if (value && value[0] )  /* NOT NULL and not empty */
            {
              if ( ! iupStrEqualNoCase(value, iupAttribGetId( data, ih, name, id)))     /* NOT already stored */
...
IUP_API void amy_IupCopyClassAttributes( struct libData *data, Ihandle *src_ih, Ihandle *dst_ih)
{
  ...
  if ( ! amy_IupClassMatch( data, dst_ih, src_ih->iclass->name))
    return;
  ...
  has_attrib_id = ic->has_attrib_id;
  if (iupClassMatch( data, ic, "tree" ) ||  /* tree can only set id attributes after map, so they can not be saved */
      iupClassMatch( data, ic, "cells" ))   /* cells does not have any savable id attributes */
    has_attrib_id = 0;  
  ...
  while (name)
  {
    ...
    if (afunc && !(afunc->flags & IUPAF_NO_STRING) &&  /* is a string */
                 ...
                 !(afunc->flags & IUPAF_CALLBACK))     /* not a callback */
    {
      if ((afunc->flags&IUPAF_NO_SAVE) && iupBaseNoSaveCheck( data, src_ih, name))  /* can not be saved */
      {
        name = iupTableNext( data, ic->attrib_func );
        continue;
      }
      ...
      if ( ! (afunc->flags & IUPAF_HAS_ID))     /* no ID */
      {
        char *value = amy_IupGetAttribute( data, src_ih, name );
        if (value && value[0] )    /* NOT NULL and not empty */
        {
          if ( ! iupStrEqualNoCase(value, amy_IupGetAttribute( data, dst_ih, name)))     /* NOT already equal */
      ...
      else if (has_attrib_id)
      {
        ...
        {
          int lin, col, 
              numcol = amy_IupGetInt( data, src_ih, "NUMCOL" )+1,
              numlin = amy_IupGetInt( data, src_ih, "NUMLIN" )+1;
          for (lin=0; lin<numlin; lin++)
          {
            for (col=0; col<numcol; col++)
            {
              value = amy_IupGetAttributeId2( data, src_ih, name, lin, col );
              if (value && value[0] )  /* NOT NULL and not empty */
              {
                if ( ! iupStrEqualNoCase(value, amy_IupGetAttributeId2( data, dst_ih, name, lin, col)))     /* NOT already stored */
                  amy_IupStoreAttributeId2( data, dst_ih, name, lin, col, value );
              }
            }
          }
        }
        ...
        {
          int id, count = amy_IupGetInt( data, src_ih, "COUNT" );
          for (id=start_id; id<count+start_id; id++)
          {
            value = amy_IupGetAttributeId( data, src_ih, name, id );
            if (value && value[0] )  /* NOT NULL and not empty */
            {
              if ( ! iupStrEqualNoCase(value, amy_IupGetAttributeId( data, dst_ih, name, id)))     /* NOT already stored */
                amy_IupStoreAttributeId( data, dst_ih, name, id, value );
            }
          }
        }
      }
  ...
  while (name)
  {
                 ...
                 !(afunc->flags & IUPAF_CALLBACK))      /* not a callback */
    {
      char *value = amy_IupGetAttribute( data, src_ih, name );
      if (value &&     /* NOT NULL */
          !iupStrEqualNoCase(value, amy_IupGetAttribute( data, dst_ih, name)))     /* NOT already stored */
        amy_IupStoreAttribute( data, dst_ih, name, value );
    }
...
void iupClassObjectEnsureDefaultAttributes( struct libData *data, Ihandle *ih )
{
  Iclass *ic;
  char *name;
  ...
  while (name)
  {
        ...
        !(afunc->flags & IUPAF_HAS_ID))
    {
      if (( ! iupStrEqualNoCase(afunc->default_value, afunc->system_default)) || 
          (afunc->call_global_default && afunc->default_value && iupGlobalDefaultColorChanged( data, afunc->default_value )))
      {
        if (( ! ih->handle &&  (afunc->flags & IUPAF_NOT_MAPPED)) ||
             (ih->handle && !(afunc->flags & IUPAF_NOT_MAPPED)))
        {
          char *value = iupAttribGet( data, ih, name );
          if ( ! value)  /* if set will be updated later */
            afunc->set( data, ih, iClassGetDefaultValue( data, afunc ));
        }
      }
    }
...
void iupClassUpdate( struct libData *data, Iclass *ic )
{
  IattribFunc* afunc = (IattribFunc*) iupTableGet( data, ic->attrib_func, "CLASSUPDATE" );
  if (afunc && afunc->set)
    afunc->set( data, (Ihandle*)ic, NULL );
}
