...
static char *iToggleGetRadioAttrib( struct libData *data, Ihandle *ih )
{
  return iupStrReturnBoolean( ih->data->is_radio );
}

static int iToggleSetFlatAttrib( struct libData *data, Ihandle *ih, const char *value)
{
...
static char *iToggleGetFlatAttrib( struct libData *data, Ihandle *ih )
{
...
static int iToggleCreateMethod( struct libData *data, Ihandle *ih, void** params)
{
  if (params)
  {
...
static void iToggleComputeNaturalSizeMethod( struct libData *data, Ihandle *ih, int *w, int *h, int *children_expand)
{
  int natural_w = 0, 
      natural_h = 0,
      type = ih->data->type;
  ...
  if (type == IUP_TOGGLE_IMAGE)
  {
    ...
    iupdrvToggleAddBorders( data, ih, &natural_w, &natural_h );
  ...
  {
    /* must use amy_IupGetAttribute to check from the native implementation */
    char *title = amy_IupGetAttribute( data, ih, "TITLE" );
    char *str = iupStrProcessMnemonic(title, NULL, 0 );   /* remove & */
    iupdrvFontGetMultiLineStringSize( data, ih, str, &natural_w, &natural_h );

    iupdrvToggleAddCheckBox( data, ih, &natural_w, &natural_h, str );
...
IUP_API Ihandle *amy_IupToggle( struct libData *data, const char *title, const char *action )
{
  ...
  params[2] = NULL;
  return amy_IupCreatev( data, "toggle", params );
}
...
Iclass *iupToggleNewClass( struct libData *data )
{
  ...
  /* Callbacks */
  iupClassRegisterCallback( data, ic, "ACTION", "i" );
  ...
  iupClassRegisterAttribute( data, ic, "RADIO", iToggleGetRadioAttrib, NULL, NULL, NULL, IUPAF_READONLY|IUPAF_NO_INHERIT );
  iupClassRegisterAttribute( data, ic, "3STATE", NULL, NULL, NULL, NULL, IUPAF_NO_INHERIT );
  iupClassRegisterAttribute( data, ic, "FLAT", iToggleGetFlatAttrib, iToggleSetFlatAttrib, IUPAF_SAMEASSYSTEM, "No", IUPAF_NOT_MAPPED );
  iupClassRegisterAttribute( data, ic, "IGNORERADIO", NULL, NULL, NULL, NULL, IUPAF_NO_INHERIT );

  iupdrvToggleInitClass( data, ic );
