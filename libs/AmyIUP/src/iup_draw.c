...
IUP_API void amy_IupDrawBegin( struct libData *data, Ihandle *ih )
{
  ...
  dc = iupdrvDrawCreateCanvas( data, ih );
  iupAttribSet( data, ih, "_IUP_DRAW_DC", (char*)dc );
}
...
IUP_API void amy_IupDrawEnd( struct libData *data, Ihandle *ih )
{
  ...
  dc = (IdrawCanvas*) iupAttribGet( data, ih, "_IUP_DRAW_DC" );
  if ( ! dc)
    return;

  iupdrvDrawFlush( data, dc );
  iupdrvDrawKillCanvas( data, dc );
  iupAttribSet( data, ih, "_IUP_DRAW_DC", NULL );
}
...
static int iDrawGetStyle( struct libData *data, Ihandle *ih )
{
  char *style = iupAttribGetStr( data, ih, "DRAWSTYLE" );
...
static int iDrawGetLineWidth( struct libData *data, Ihandle *ih )
{
  int line_width = iupAttribGetInt( data, ih, "DRAWLINEWIDTH" );
  if (line_width == 0)
    return 1;
  else
    return line_width;
...
IUP_API void amy_IupDrawLine( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2)
{
  ...
  color = iupDrawStrToColor( data, iupAttribGetStr( data, ih, "DRAWCOLOR" ), 0 );

  line_width = iDrawGetLineWidth( data, ih );
  style = iDrawGetStyle( data, ih );

  iupdrvDrawLine( data, dc, x1, y1, x2, y2, color, style, line_width );
}
...
IUP_API void amy_IupDrawRectangle( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2 )
{
  ...
  iupdrvDrawRectangle( data, dc, x1, y1, x2, y2, color, style, line_width );
}
...
IUP_API void amy_IupDrawArc( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2, double a1, double a2)
{
  ...
  iupdrvDrawArc( data, dc, x1, y1, x2, y2, a1, a2, color, style, line_width );
}

IUP_API void amy_IupDrawPolygon( struct libData *data, Ihandle *ih, int* points, int count)
{
  IdrawCanvas* dc;
  long color = 0;
  int style, line_width;
  ...
  iupdrvDrawPolygon( data, dc, points, count, color, style, line_width );
}

static void iDrawRotatePoint(
	int x, int y, int *rx, int *ry, double sin_theta, double cos_theta)
{
  double t;
...
static void iDrawGetTextBounds(
	int w, int h, double text_orientation, int *o_w, int *o_h)
{
...
IUP_SDK_API char *iupDrawGetTextSize( struct libData *data, Ihandle *ih, const char *text, int len, int *w, int *h, double text_orientation)
{
  char*font = iupAttribGetStr( data, ih, "DRAWFONT" );
  if ( ! font)
  ...
  if (text_orientation)
  {
    if (text_orientation == 90)
      iupdrvFontGetTextSize( data, font, text, len, h, w );
    else
    {
      int txt_w, txt_h;
      iupdrvFontGetTextSize( data, font, text, len, &txt_w, &txt_h );
      iDrawGetTextBounds(txt_w, txt_h, text_orientation, w, h );
    }
  }
  else
    iupdrvFontGetTextSize( data, font, text, len, w, h );
...
{
  int flags = iupFlatGetHorizontalAlignment( data, iupAttribGetStr( data, ih, align_name));
  int wrap = iupAttribGetBoolean( data, ih, wrap_name );
  int ellipsis = iupAttribGetBoolean( data, ih, ellipsis_name );
  if (wrap)
    flags |= IUP_DRAW_WRAP;
  if (ellipsis)
...
IUP_API void amy_IupDrawText( struct libData *data, Ihandle *ih, const char *text, int len, int x, int y, int w, int h)
{
  IdrawCanvas* dc;
  long color = 0;
  int text_flags;
  ...
  color = iupDrawStrToColor( data, iupAttribGetStr( data, ih, "DRAWCOLOR" ), 0 );
  text_orientation = iupAttribGetDouble( data, ih, "DRAWTEXTORIENTATION" );
  text_flags = iupDrawGetTextFlags( data, ih, "DRAWTEXTALIGNMENT", "DRAWTEXTWRAP", "DRAWTEXTELLIPSIS" );
  if (iupAttribGetBoolean( data, ih, "DRAWTEXTCLIP" ))
    text_flags |= IUP_DRAW_CLIP;
  if (iupAttribGetBoolean( data, ih, "DRAWTEXTLAYOUTCENTER" ))
  ...
  {
    int txt_w, txt_h;
    char *font = iupDrawGetTextSize( data, ih, text, len, &txt_w, &txt_h, text_orientation );
    if (w == -1 || w == 0) w = txt_w;
    if (h == -1 || h == 0) h = txt_h;
    iupdrvDrawText( data, dc, text, len, x, y, w, h, color, font, text_flags, text_orientation );
  }
}

IUP_API void amy_IupDrawGetTextSize( struct libData *data, Ihandle *ih, const char *text, int len, int *w, int *h)
{
  double text_orientation;
  ...
  iupDrawGetTextSize( data, ih, text, len, w, h, text_orientation );
}

IUP_API void amy_IupDrawGetImageInfo( struct libData *data, 
	const char *name, int *w, int *h, int *bpp)
{
  iupImageGetInfo( data, name, w, h, bpp );
}
...
IUP_API void amy_IupDrawImage( struct libData *data, Ihandle *ih, const char *name, int x, int y, int w, int h)
{
  ...
  bgcolor = iupAttribGetStr( data, ih, "DRAWBGCOLOR" );
  make_inactive = iupAttribGetInt( data, ih, "DRAWMAKEINACTIVE" );

  iupdrvDrawImage( data, dc, name, make_inactive, bgcolor, x, y, w, h );
}

IUP_API void amy_IupDrawSetClipRect( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2)
{
  IdrawCanvas* dc;
  ...
  iupdrvDrawSetClipRect( data, dc, x1, y1, x2, y2 );
}

IUP_API void amy_IupDrawGetClipRect( struct libData *data, Ihandle *ih, int *x1, int *y1, int *x2, int *y2)
{
  IdrawCanvas* dc;
  ...
  iupdrvDrawGetClipRect( data, dc, x1, y1, x2, y2 );
}

IUP_API void amy_IupDrawResetClip( struct libData *data, Ihandle *ih )
{
  IdrawCanvas* dc;
...
IUP_API void amy_IupDrawSelectRect( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2 )
{
  IdrawCanvas* dc;
  ...
  iupdrvDrawSelectRect( data, dc, x1, y1, x2, y2 );
}
...
IUP_API void amy_IupDrawFocusRect( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2 )
{
  ...
  iupdrvDrawFocusRect( data, dc, x1, y1, x2, y2 );
}
...
{
  iupDrawSetColor( data, ih, "DRAWCOLOR", light_shadow );
  amy_IupDrawLine( data, ih, x1, y1, x1, y2 );
  amy_IupDrawLine( data, ih, x1, y1, x2, y1 );

  iupDrawSetColor( data, ih, "DRAWCOLOR", mid_shadow );
  amy_IupDrawLine( data, ih, x1 + 1, y2 - 1, x2 - 1, y2 - 1 );
  amy_IupDrawLine( data, ih, x2 - 1, y1 + 1, x2 - 1, y2 - 1 );

  iupDrawSetColor( data, ih, "DRAWCOLOR", dark_shadow );
  amy_IupDrawLine( data, ih, x1, y2, x2, y2 );
  amy_IupDrawLine( data, ih, x2, y1, x2, y2 );
}
...
{
  iupDrawSetColor( data, ih, "DRAWCOLOR", dark_shadow );
  amy_IupDrawLine( data, ih, x - 1, y1, x - 1, y2 );
  iupDrawSetColor( data, ih, "DRAWCOLOR", light_shadow );
  amy_IupDrawLine( data, ih, x, y1, x, y2 );
}
...
{
  iupDrawSetColor( data, ih, "DRAWCOLOR", dark_shadow );
  amy_IupDrawLine( data, ih, x1, y - 1, x2, y - 1 );
  iupDrawSetColor( data, ih, "DRAWCOLOR", light_shadow );
  amy_IupDrawLine( data, ih, x1, y, x2, y );
}
...
IUP_SDK_API void iupDrawSunkenRect( struct libData *data, Ihandle *ih, int x1, int y1, int x2, int y2, long light_shadow, long mid_shadow, long dark_shadow)
{
  ...
  iupDrawSetColor( data, ih, "DRAWCOLOR", dark_shadow );
  amy_IupDrawLine( data, ih, x1 + 1, y1 + 1, x1 + 1, y2 - 1 );
  amy_IupDrawLine( data, ih, x1 + 1, y1 + 1, x2 - 1, y1 + 1 );
...
IUP_SDK_API void iupDrawCalcShadows( struct libData *data, 
	long bgcolor, long *light_shadow, long *mid_shadow, long *dark_shadow)
{
...
IUP_SDK_API void iupDrawParentBackground( struct libData *data, IdrawCanvas *dc, Ihandle *ih)
{
  long color;
  int w, h;
  char *color_str = iupBaseNativeParentGetBgColorAttrib( data, ih );
  color = iupDrawStrToColor( data, color_str, 0 );
  iupdrvDrawGetSize( data, dc, &w, &h );
  iupdrvDrawRectangle( data, dc, 0, 0, w - 1, h - 1, color, IUP_DRAW_FILL, 1 );
}


/***********************************************************************************************/

static long iFlatDrawColorMakeInactive( struct libData *data, long color, const char *bgcolor)
{
...
IUP_SDK_API void iupFlatDrawBorder( struct libData *data, IdrawCanvas *dc, int xmin, int xmax, int ymin, int ymax, int border_width, const char *fgcolor, const char *bgcolor, int active)
{
  long color = 0;
  ...
  color = iupDrawStrToColor( data, fgcolor, 0 );
  if ( ! active)
    color = iFlatDrawColorMakeInactive( data, color, bgcolor );

  iupdrvDrawRectangle( data, dc, xmin, ymin, xmax, ymax, color, IUP_DRAW_STROKE, 1 );
  while (border_width > 1)
  {
    border_width--;
    iupdrvDrawRectangle( data, dc, xmin + border_width,
                        ymin + border_width,
                        xmax - border_width,
...
IUP_SDK_API void iupFlatDrawBox( struct libData *data, IdrawCanvas *dc, int xmin, int xmax, int ymin, int ymax, const char *fgcolor, const char *bgcolor, int active)
{
  long color;
  ...
  iupdrvDrawRectangle( data, dc, xmin, ymin, xmax, ymax, color, IUP_DRAW_FILL, 1 );
}

static void iFlatDrawText( struct libData *data, IdrawCanvas *dc, int x, int y, int w, int h, const char *str, const char *font, int text_flags, double text_orientation, const char *fgcolor, const char *bgcolor, int active)
{
  long color;
  ...
  iupdrvDrawText( data, dc, str, (int)strlen(str), x, y, w, h, color, font, text_flags | IUP_DRAW_LAYOUTCENTER, text_orientation );  /* layout is always center here */
}

static void iFlatGetIconPosition( struct libData *data, 
	int icon_width, int icon_height, int *x, int *y, int width, int height, int horiz_alignment, int vert_alignment)
{
...
static void iFlatGetImageTextPosition( struct libData *data, 
	int x, int y, int img_position, int spacing,
                            ...
                            const char *imagename, const char *title, int *w, int *h, double text_orientation)
{
  if (imagename)
  {
    ...
    iupImageGetInfo( data, imagename, &img_width, &img_height, NULL );

    if (title)
    {
      int txt_width, txt_height;
      iupDrawGetTextSize( data, ih, title, 0, &txt_width, &txt_height, text_orientation );
  ...
  *h += 2 * vert_padding;
  ...
  if (ih->iclass->is_interactive && iupAttribGetBoolean( data, ih, "CANFOCUS" ) && iupAttribGetBoolean( data, ih, "FOCUSFEEDBACK" ))
  {
                     ...
                     const char *imagename, int make_inactive, const char *title, int text_flags, double text_orientation, const char *fgcolor, const char *bgcolor, int active)
{
  ...
  iupdrvDrawGetClipRect( data, dc, &clip_x1, &clip_y1, &clip_x2, &clip_y2 );
  if (clip_x1 != 0 || clip_y1 != 0 || clip_x2 != 0 || clip_y2)
    iupdrvDrawSetClipRect( data, dc, iupMAX(icon_x, clip_x1), iupMAX(icon_y, clip_y1), iupMIN(icon_x + icon_width, clip_x2), iupMIN(icon_y + icon_height, clip_y2));  /* intersect */
  else
    iupdrvDrawSetClipRect( data, dc, icon_x, icon_y, icon_x + icon_width, icon_y + icon_height );
  ...
  if (imagename)
  {
    ...
    if (title)
    {
      ...
      font = iupDrawGetTextSize( data, ih, title, 0, &txt_width, &txt_height, text_orientation );
      ...
      iFlatGetIconPosition( data, icon_width, icon_height, &x, &y, width, height, horiz_alignment, vert_alignment );

      iFlatGetImageTextPosition( data, x, y, img_position, spacing,
                                img_width, img_height, txt_width, txt_height,
                                &img_x, &img_y, &txt_x, &txt_y );

      iupdrvDrawImage( data, dc, imagename, make_inactive, bgcolor, icon_x + img_x, icon_y + img_y, img_width, img_height );  /* no zoom */
      iFlatDrawText( data, dc, icon_x + txt_x, icon_y + txt_y, txt_width, txt_height, title, font, text_flags, text_orientation, fgcolor, bgcolor, active );
    }
    else
    {
      ...
      iupdrvDrawImage( data, dc, imagename, make_inactive, bgcolor, icon_x + x, icon_y + y, img_width, img_height );  /* no zoom */
    }
  }
  else if (title)
  {
    ...
    iFlatDrawText( data, dc, icon_x + x, icon_y + y, width, height, title, font, text_flags, text_orientation, fgcolor, bgcolor, active );
  }

  iupdrvDrawSetClipRect( data, dc, clip_x1, clip_y1, clip_x2, clip_y2 );

  if (title && !iupAttribGet( data, ih, "SECONDARYTITLE" ))
    iupdrvSetAccessibleTitle( data, ih, title );  /* for accessibility */
}
...
IUP_SDK_API void iupFlatDrawArrow( struct libData *data, IdrawCanvas *dc, int x, int y, int size, const char *color_str, const char *bgcolor, int active, int dir)
{
  int points[6];

  int off1 = iupRound( data, (double)size * 0.13 );
  int off2 = iupRound( data, (double)size * 0.87 );
  int half = size / 2;

  long color = iupDrawStrToColor( data, color_str, 0 );
  if ( ! active)
    color = iFlatDrawColorMakeInactive( data, color, bgcolor );
  ...
  iupdrvDrawPolygon( data, dc, points, 3, color, IUP_DRAW_FILL, 1 );
  iupdrvDrawPolygon( data, dc, points, 3, color, IUP_DRAW_STROKE, 1 );
}
...
IUP_SDK_API void iupFlatDrawCheckMark( struct libData *data, IdrawCanvas *dc, int xmin, int xmax, int ymin, int ymax, const char *color_str, const char *bgcolor, int active)
{
  ...
  iupdrvDrawPolygon( data, dc, points, 3, color, IUP_DRAW_STROKE, 2 );
}
...
IUP_SDK_API void iupFlatDrawDrawCircle( struct libData *data, IdrawCanvas *dc, int xc, int yc, int radius, int fill, int line_width, char *fgcolor, char *bgcolor, int active)
{
  ...
  long color = iupDrawStrToColor( data, fgcolor, 0 );
  if ( ! active)
    color = iFlatDrawColorMakeInactive( data, color, bgcolor );
  ...
  iupdrvDrawArc( data, dc, x1, y1, x2, y2, 0.0, 360, color, style, line_width );
}

static char *iFlatDrawGetImageName( struct libData *data, Ihandle *ih, const char *baseattrib, const char *state)
{
  char attrib[1024];
  strcpy(attrib, baseattrib );
  strcat(attrib, state );
  return iupAttribGetStr( data, ih, attrib );
}
...
IUP_SDK_API const char *iupFlatGetImageName( struct libData *data, Ihandle *ih, const char *baseattrib, const char *basevalue, int press, int highlight, int active, int *make_inactive)
{
  ...
  *make_inactive = 0;
  ...
  {
    if (press)
      imagename = iFlatDrawGetImageName( data, ih, baseattrib, "PRESS" );
    else
    {
      if (highlight)
        imagename = iFlatDrawGetImageName( data, ih, baseattrib, "HIGHLIGHT" );
    }
  }
  else
  {
    imagename = iFlatDrawGetImageName( data, ih, baseattrib, "INACTIVE" );
    if ( ! imagename)
      *make_inactive = 1;
  }
  ...
  {
    if ( ! basevalue)
      basevalue = iupAttribGetStr( data, ih, baseattrib );
...
static char *iFlatDrawGetImageNameId( struct libData *data, Ihandle *ih, const char *baseattrib, const char *state, int id)
{
  char attrib[1024];
  strcpy(attrib, baseattrib );
  strcat(attrib, state );
  return iupAttribGetId( data, ih, attrib, id );
}
...
IUP_SDK_API const char *iupFlatGetImageNameId( struct libData *data, Ihandle *ih, const char *baseattrib, int id, const char *basevalue, int press, int highlight, int active, int *make_inactive)
{
  ...
  *make_inactive = 0;
  ...
  {
    if (press == id)
      imagename = iFlatDrawGetImageNameId( data, ih, baseattrib, "PRESS", id );
    else
    {
      if (highlight == id)
        imagename = iFlatDrawGetImageNameId( data, ih, baseattrib, "HIGHLIGHT", id );
    }
  }
  else
  {
    imagename = iFlatDrawGetImageNameId( data, ih, baseattrib, "INACTIVE", id );
    if ( ! imagename)
      *make_inactive = 1;
  }
  ...
  {
    if ( ! basevalue)
      basevalue = iupAttribGetId( data, ih, baseattrib, id );
...
{
  char *value = iupAttribGet( data, ih, "DARK_DLGBGCOLOR" );
  if ( ! value)
  {
    unsigned char r, g, b;
    iupStrToRGB( amy_IupGetGlobal( data, "DLGBGCOLOR" ), &r, &g, &b );
    r = (r * 90) / 100;
    g = (g * 90) / 100;
    b = (b * 90) / 100;
    iupAttribSetStrf( data, ih, "DARK_DLGBGCOLOR", "%d %d %d", r, g, b );
    return iupAttribGet( data, ih, "DARK_DLGBGCOLOR" );
  }
  else
    return value;
}
...
static void iFlatItemSetTipVisible( struct libData *data, Ihandle *ih, const char *tip)
{
  int visible = amy_IupGetInt( data, ih, "TIPVISIBLE" );

  /* do not call amy_IupSetAttribute */
  iupAttribSetStr( data, ih, "TIP", tip );
  iupdrvBaseSetTipAttrib( data, ih, tip );

  if (visible)
  {
    amy_IupSetAttribute( data, ih, "TIPVISIBLE", "No" );
    if (tip)
      amy_IupSetAttribute( data, ih, "TIPVISIBLE", "Yes" );
  }
}

static int iFlatItemCheckTip( struct libData *data, Ihandle *ih, const char *new_tip)
{
  char *tip = iupAttribGet( data, ih, "TIP" );
  if ( ! tip && !new_tip)
    return 1;
...
{
  char *tip = iupAttribGet( data, ih, "_IUP_FLATITEM_TIP" );
  if ( ! iFlatItemCheckTip( data, ih, tip))
    iFlatItemSetTipVisible( data, ih, tip );
}
...
{
  iupAttribSetStr( data, ih, "_IUP_FLATITEM_TIP", value );
  return iupdrvBaseSetTipAttrib( data, ih, value );
}
