...
const char *iupFontGetPangoName( struct libData *data, const char *name )
{
  int i;
  if ( ! name)
    return NULL;
...
const char *iupFontGetWinName( struct libData *data, const char *name )
{
  int i;
  if ( ! name)
    return NULL;
...
const char *iupFontGetXName( struct libData *data, const char *name )
{
  int i;
  if ( ! name)
    return NULL;
...
const char *iupFontGetMacName( struct libData *data, const char *name )
{
  int i;
  if ( ! name)
    return NULL;
...
{
  char *value = iupAttribGetStr( data, ih, "FONT" );

  /* compensate the fact that FONT in amy_IupMatrix is used with Id2 (lin,col) and does NOT have a default value. */
  if ( ! value && ih->iclass->nativetype == IUP_TYPECANVAS)
  {
    value = iupAttribGetInherit( data, ih, "FONT" );
    if ( ! value)
      return amy_IupGetGlobal( data, "DEFAULTFONT" );
  }
...
{
  iupAttribSetClassObject( data, ih, "FONT", iupGetFontValue( data, ih ));
}
...
IUP_SDK_API int iupGetFontInfo( struct libData *data, const char *font, char *typeface, int *size, int *is_bold, int *is_italic, int *is_underline, int *is_strikeout)
{
  ...
  *typeface = 0;
  ...
  if ( ! iupFontParseWin( data, font, typeface, size, is_bold, is_italic, is_underline, is_strikeout))
  {
    if ( ! iupFontParseX( data, font, typeface, size, is_bold, is_italic, is_underline, is_strikeout))
    {
      if ( ! iupFontParsePango( data, font, typeface, size, is_bold, is_italic, is_underline, is_strikeout))
        return 0;
    }
  }
...
IUP_SDK_API int iupSetFontFaceAttrib( struct libData *data, Ihandle *ih, const char *value)
{
  ...
  amy_IupSetfAttribute( data, ih, "FONT", "%s, %s%s%s%s %d", value, is_bold ? "Bold " : "", is_italic ? "Italic " : "", is_underline ? "Underline " : "", is_strikeout ? "Strikeout " : "", size );
  return 0;
}
...
IUP_SDK_API int iupSetFontSizeAttrib( struct libData *data, Ihandle *ih, const char *value)
{
  ...
  amy_IupSetfAttribute( data, ih, "FONT", "%s, %s%s%s%s %s", typeface, is_bold?"Bold ":"", is_italic?"Italic ":"", is_underline?"Underline ":"", is_strikeout?"Strikeout ":"", value );
  return 0;
}
...
IUP_SDK_API char *iupGetFontStyleAttrib( struct libData *data, Ihandle *ih )
{
  ...
  int is_bold = 0,
    ...
    is_strikeout = 0;
  char typeface[1024];
  char *font = iupGetFontValue( data, ih );
...
IUP_SDK_API int iupSetFontStyleAttrib( struct libData *data, Ihandle *ih, const char *value)
{
  ...
  amy_IupSetfAttribute( data, ih, "FONT", "%s, %s %d", typeface, value, size );
...
char *iupGetDefaultFontFaceGlobalAttrib( struct libData *data )
{
  ...
  char *font = amy_IupGetGlobal( data, "DEFAULTFONT" );
...
int iupSetDefaultFontFaceGlobalAttrib( struct libData *data, const char *value )
{
	...
	font = amy_IupGetGlobal( data, "DEFAULTFONT" );

	if ( ! iupGetFontInfo( data, font, typeface, &size, &is_bold, &is_italic, &is_underline, &is_strikeout))
		return 0;

	amy_IupSetfAttribute( data, NULL, "DEFAULTFONT", "%s, %s%s%s%s %d", value, is_bold ? "Bold " : "", is_italic ? "Italic " : "", is_underline ? "Underline " : "", is_strikeout ? "Strikeout " : "", size );
...
int iupSetDefaultFontStyleGlobalAttrib( struct libData *data, const char *value )
{
	...
	amy_IupSetfAttribute( data, NULL, "DEFAULTFONT", "%s, %s %d", typeface, value, size );
...
void iupSetDefaultFontSizeGlobalAttrib( struct libData *data, const char *value )
{
	...
	amy_IupSetfAttribute( data, NULL, "DEFAULTFONT", "%s, %s%s%s%s %s", typeface, is_bold?"Bold ":"", is_italic?"Italic ":"", is_underline?"Underline ":"", is_strikeout?"Strikeout ":"", value );
...
static int iFontFindStyleName( struct libData *data, const char *name, int len, int *style )
{
...
/* this code is shared between CD and IUP, must be updated on both libraries */
static const char * iFontGetStyleWord( struct libData *data, const char *str, const char *last, int *wordlen )
{
  const char *result;
...
/* this code is shared between CD and IUP, must be updated on both libraries */
IUP_SDK_API int iupFontParsePango( struct libData *data, const char *font, char *typeface, int *size, int *bold, int *italic, int *underline, int *strikeout )
{
  const char *p, *last;
  ...
  len = (int)strlen(font );
  last = font + len;
  p = iFontGetStyleWord( data, font, last, &wordlen );
  ...
  while (wordlen != 0)
  {
    ...
    if ( ! iFontFindStyleName( data, p, wordlen, &new_style))
      break;
    else
    {
      style |= new_style;
...
/* this code is shared between CD and IUP, must be updated on both libraries */
IUP_SDK_API int iupFontParseX( struct libData *data, const char *font, char *typeface, int *size, int *bold, int *italic, int *underline, int *strikeout )
{
