...
IUP_SDK_API int iupMaskCheck( struct libData *data, Imask *mask, const char *val)
{
  int ret;
  ...
  ret = iupMaskMatch( data, val, mask->fsm, 0, NULL, NULL, NULL, mask->casei );
...
IUP_SDK_API Imask *iupMaskCreate( struct libData *data, const char* mask_str )
{
  ...
  if (iupMaskParse( data, copy_mask_str, &fsm ) != IMASK_PARSE_OK)
  {
    free(copy_mask_str);
    return NULL;
  }
...
IUP_SDK_API Imask *iupMaskCreateInt( struct libData *data, int min, int max )
{
  ...
  if (min < 0)
    mask = iupMaskCreate( data, IUP_MASK_INT );
  else
    mask = iupMaskCreate( data, IUP_MASK_UINT );
...
IUP_SDK_API Imask *iupMaskCreateReal( struct libData *data, int positive, const char* decimal_symbol )
{
  ...
  {
    if (positive)
      mask = iupMaskCreate( data, IUP_MASK_UFLOATCOMMA );
    else
      mask = iupMaskCreate( data, IUP_MASK_FLOATCOMMA );
  }
  else
  {
    if (positive)
      mask = iupMaskCreate( data, IUP_MASK_UFLOAT );
    else
      mask = iupMaskCreate( data, IUP_MASK_FLOAT );
  }
...
{
  Imask *mask = iupMaskCreateReal( data, min >= 0, decimal_symbol );
