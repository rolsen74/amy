...
static int iLabelSetSeparatorAttrib( struct libData *data, Ihandle *ih, const char *value)
{
...
static char *iLabelGetSeparatorAttrib( struct libData *data, Ihandle *ih )
{
  if (ih->handle)
  {
...
int iupLabelGetTypeBeforeMap( struct libData *data, Ihandle *ih )
{
  ...
  if ( ! ih->handle)
  {
    ...
    char *value = iupAttribGet( data, ih, "SEPARATOR" );
    if (value)
    {
      if (iupStrEqualNoCase(value, "HORIZONTAL" ))
    ...
    {
      value = iupAttribGet( data, ih, "IMAGE" );
      if (value)
...
char *iupLabelGetPaddingAttrib( struct libData *data, Ihandle *ih )
{
  ...
  int type = iupLabelGetTypeBeforeMap( data, ih );
...
static int iLabelCreateMethod( struct libData *data, Ihandle *ih, void** params)
{
  if (params && params[0] )
...
static void iLabelComputeNaturalSizeMethod( struct libData *data, Ihandle *ih, int *w, int *h, int *children_expand)
{
  int natural_w = 0, 
      natural_h = 0, 
      type = iupLabelGetTypeBeforeMap( data, ih );
  ...
  iupdrvLabelAddExtraPadding( data, ih, &natural_w, &natural_h );
...
IUP_API Ihandle *amy_IupLabel( struct libData *data, const char *title )
{
  ...
  params[0] = (void*)title;
  params[1] = NULL;
  return amy_IupCreatev( data, "label", params );
}
...
Iclass *iupLabelNewClass( struct libData *data )
{
  ...
  /* amy_IupLabel only */
  iupClassRegisterAttribute( data, ic, "SEPARATOR", iLabelGetSeparatorAttrib, iLabelSetSeparatorAttrib, NULL, NULL, IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT );
