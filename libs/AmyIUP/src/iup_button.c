...
static int iButtonSetImagePositionAttrib( struct libData *data, Ihandle *ih, const char *value)
{
...
static char *iButtonGetImagePositionAttrib( struct libData *data, Ihandle *ih)
{
  char *img_pos2str[4] = {"LEFT", "RIGHT", "TOP", "BOTTOM"};
  return iupStrReturnStr(img_pos2str[ih->data->img_position] );
}

static int iButtonSetFocusOnClickAttrib( struct libData *data, Ihandle *ih, const char *value)
{
  iupAttribSet( data, ih, "CANFOCUS", value );
  return 1;
}

static int iButtonSetSpacingAttrib( struct libData *data, Ihandle *ih, const char *value)
{
  if ( ! ih->handle)  /* set only before map */
...
static char *iButtonGetSpacingAttrib( struct libData *data, Ihandle *ih)
{
...
static int iButtonCreateMethod( struct libData *data, Ihandle *ih, void** params)
{
  if (params)
  {
...
static void iButtonComputeNaturalSizeMethod( struct libData *data, Ihandle *ih, int *w, int *h, int *children_expand)
{
  int has_border = 1;
  int natural_w = 0,
      natural_h = 0, 
  ...
  if ( ! ih->handle)
  {
    ...
    char *value = iupAttribGet( data, ih, "IMAGE" );
    if (value)
    {
  ...
  {
    iupImageGetInfo( data, iupAttribGet( data, ih, "IMAGE" ), &natural_w, &natural_h, NULL );
    ...
    if (type & IUP_BUTTON_TEXT)
    {
      ...
      iupdrvFontGetMultiLineStringSize( data, ih, str, &text_w, &text_h );
  ...
  else /* IUP_BUTTON_TEXT only */
  {
    char *title = iupAttribGet( data, ih, "TITLE" );
    char *str = iupStrProcessMnemonic( title, NULL, 0 );   /* remove & */
    iupdrvFontGetMultiLineStringSize( data, ih, str, &natural_w, &natural_h );
    if (str && str!=title) free(str );
  }

  if (ih->data->type & IUP_BUTTON_IMAGE &&
      iupAttribGet( data, ih, "IMPRESS" ) &&
      !iupAttribGetBoolean( data, ih, "IMPRESSBORDER" ))
    has_border = 0;

  if (has_border)
    iupdrvButtonAddBorders( data, ih, &natural_w, &natural_h );
...
IUP_API Ihandle *amy_IupButton( struct libData *data, const char *title, const char *action )
{
  ...
  params[2] = NULL;
  return amy_IupCreatev( data, "button", params );
}
...
Iclass *iupButtonNewClass( struct libData *data )
{
  ...
  /* amy_IupButton only */
  iupClassRegisterAttribute( data, ic, "SPACING", iButtonGetSpacingAttrib, iButtonSetSpacingAttrib, IUPAF_SAMEASSYSTEM, "2", IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT );
  ...
  iupClassRegisterAttribute( data, ic, "IMAGEPOSITION", iButtonGetImagePositionAttrib, iButtonSetImagePositionAttrib, IUPAF_SAMEASSYSTEM, "LEFT", IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT );
  iupClassRegisterAttribute( data, ic, "IMPRESSBORDER", NULL, NULL, NULL, NULL, IUPAF_NO_INHERIT );
  iupClassRegisterAttribute( data, ic, "FLAT", NULL, NULL, NULL, NULL, IUPAF_DEFAULT );
  iupClassRegisterAttribute( data, ic, "FOCUSONCLICK", NULL, iButtonSetFocusOnClickAttrib, "YES", NULL, IUPAF_NOT_MAPPED|IUPAF_NO_INHERIT );
