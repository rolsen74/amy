...
static int iMaskParseExpression( struct libData *data, ImaskParseVars *vars );
static int iMaskParseTerm( struct libData *data, ImaskParseVars *vars );
static int iMaskParseFactor( struct libData *data, ImaskParseVars *vars );
static void iMaskParseError( struct libData *data, ImaskParseVars *vars);
static void iMaskParseNewState( struct libData *data, ImaskParseVars *vars );
static void iMaskParseSetState( struct libData *data, ImaskParseVars *vars, int state, char ch, char command, int next1, int next2 );

int iupMaskSetChar( struct libData *data, int char_number, char new_char )
{
...
int iupMaskParse( struct libData *data, const char *text, ImaskParsed **fsm )
{
  int t;
  ImaskParseVars vars;
  ...
  if (setjmp (vars.env) == 0)
    t = iMaskParseExpression( data, &vars );
  ...
  iMaskParseSetState( data, &vars, 0, vars.fsm[0].ch, IMASK_NULL_CMD, t, vars.state + 1);
  iMaskParseSetState( data, &vars, vars.state, 0, IMASK_NULL_CMD, 0, 0);
...
static int iMaskParseExpression( struct libData *data, ImaskParseVars *vars )
{
  ...
  if (vars->string[vars->j] == OR_CH)
  {
    ...
    vars->j++;
    iMaskParseNewState( data, vars );

    t3 = iMaskParseExpression( data, vars );  /* pega o 2o ramo do OR */
    ...
    iMaskParseSetState( data, vars, t2, 0, IMASK_NULL_CMD, t1, t3);
    iMaskParseSetState( data, vars, vars->state, 0, IMASK_NULL_CMD, vars->state + 1, vars->state + 1);
...
static int iMaskParseFactor( struct libData *data, ImaskParseVars *vars )
{
  ...
  {
    vars->j++;
    t2 = iMaskParseExpression( data, vars );
  ...
  else if (vars->string[vars->j] == CAP_OPEN_CH)
  {
    ...
    iMaskParseSetState( data, vars, vars->state, vars->nextcap, 
    ...
    iMaskParseExpression( data, vars );

    if (vars->string[vars->j] == CAP_CLOSE_CH)
    {
      iMaskParseSetState( data, vars, vars->state, (char)vars->capture[vars->size--],
  ...
  {
    iMaskParseSetState( data, vars, vars->state, 1, IMASK_ANY_CMD, vars->state + 1, vars->state + 1);
    t2 = vars->state;
    vars->j++;
    iMaskParseNewState( data, vars );
  }
  ...
  else if (vars->string[vars->j] == NEG_CH)
  {
    ...
    vars->j++;
    iMaskParseNewState( data, vars );
    t6 = iMaskParseFactor( data, vars );
    iMaskParseSetState( data, vars, t2, 1, IMASK_NEG_OPEN_CMD, t6, vars->state);
    iMaskParseSetState( data, vars, vars->state, 1, IMASK_NEG_CLOSE_CMD, vars->state + 1, vars->state + 1);
    iMaskParseNewState( data, vars );
  }

  else if (vars->string[vars->j] == BEGIN_CH)
  {
    iMaskParseSetState( data, vars, vars->state, 1, IMASK_BEGIN_CMD, vars->state + 1,
      vars->state + 1);
    t2 = vars->state;
    vars->j++;
    iMaskParseNewState( data, vars );
  ...
  {
    iMaskParseSetState( data, vars, vars->state, 1, IMASK_END_CMD, vars->state + 1,
      vars->state + 1);
    t2 = vars->state;
    vars->j++;
    iMaskParseNewState( data, vars );
  ...
  {
    iMaskParseSetState( data, vars, vars->state, vars->string[vars->j], 
  ...
  {
    vars->j++;
    iMaskParseSetState( data, vars, vars->state, 0, IMASK_CLASS_CMD, 0, 0);
      ...
      && (vars->string[vars->j] != '\0'))
    {
      if (vars->string[vars->j] == SPC_CH)
      {
        ...
        iMaskParseSetState( data, vars, vars->state, temp, IMASK_CLASS_CMD_CHAR, 0, 0);
        vars->j++;
        iMaskParseNewState( data, vars );
      }
      else if (vars->string[vars->j] == SEPCL_CH)
      {
        ...
        iMaskParseSetState( data, vars, vars->state - 1, vars->fsm[vars->state - 1].ch,
                     IMASK_CLASS_CMD_RANGE, temp, 0);
        vars->j++;
      }
      else if (vars->string[vars->j] == BEGIN_CH)
      {
        iMaskParseSetState( data, vars, vars->state, 1, IMASK_BEGIN_CMD, vars->state + 1, vars->state + 1);
        t2 = vars->state;
        vars->j++;
        iMaskParseNewState( data, vars );
      }
      ...
      {
        iMaskParseSetState( data, vars, vars->state, 1, IMASK_END_CMD, vars->state + 1, vars->state + 1);
        t2 = vars->state;
        vars->j++;
        iMaskParseNewState( data, vars );
      }
      ...
      {
        iMaskParseSetState( data, vars, vars->state, vars->string[vars->j], IMASK_CLASS_CMD_CHAR, 0, 0);
        vars->j++;
        iMaskParseNewState( data, vars );
      }
  ...
  {
    int loop1 = 0;
    ImaskMatchFunc *match_functions = iupMaskMatchGetFuncs( data );
    ...
    if (match_functions[loop1].ch == '\0')
    {
      ...
      iMaskParseSetState( data, vars, vars->state, (char)temp, IMASK_CHAR_CMD, vars->state + 1, vars->state + 1);
    }

    else
    {
      iMaskParseSetState( data, vars, vars->state, (char)loop1, IMASK_SPC_CMD, vars->state + 1, vars->state + 1);
    }
  ...
  {
    iMaskParseSetState( data, vars, vars->state, 0, IMASK_NULL_CMD, vars->state + 1, t2);
    r = vars->state;
