...
S32 error;
	...
	IExec->DebugPrintF( "AmySysLog : ROMInit 1\n" );

	NewlibBase = IExec->OpenLibrary( "newlib.library", 53 );

	if ( ! NewlibBase )
	{
		IExec->DebugPrintF( "AmySysLog : Error opening newlib v53" );
		goto bailout;
	}
	...
	if ( ! INewlib )
	{
		IExec->DebugPrintF( "AmySysLog : Error opening newlib interface" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmySysLog : ROMInit 2\n" );
	...
	if ( ! libBase )
	{
		IExec->DebugPrintF( "AmySysLog : Error creating Device" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmySysLog : ROMInit 3\n" );
	...
	IExec->DebugPrintF( "AmySysLog : Rock'n Roll\n" );
	error = FALSE;
