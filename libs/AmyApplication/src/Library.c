...
S32 error;
	...
	IExec->DebugPrintF( "AmyApplication : ROMInit 1\n" );

	NewlibBase = IExec->OpenLibrary( "newlib.library", 53 );

	if ( ! NewlibBase )
	{
		IExec->DebugPrintF( "AmyApplication : Error opening newlib v53" );
		goto bailout;
	}
	...
	if ( ! INewlib )
	{
		IExec->DebugPrintF( "AmyApplication : Error opening newlib interface" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyApplication : ROMInit 2\n" );
	...
	if ( ! libBase )
	{
		IExec->DebugPrintF( "AmyApplication : Error creating Device" );
		goto bailout;
	}

	IExec->DebugPrintF( "AmyApplication : ROMInit 3\n" );
	...
	IExec->DebugPrintF( "AmyApplication : Rock'n Roll\n" );
	error = FALSE;
