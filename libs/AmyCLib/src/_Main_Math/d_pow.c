...
//	EXTRACT_WORDS(hy,ly,y);
	...
	if ( ly == 0 )
	{
		...
		if ( hy == 0x3fe00000 )
		{
			...
			if ( hx >= 0 )	 /* x >= +0 */
			{
				return Self->math_d_sqrt(x);
			}
		}
	}

	ax = Self->math_d_fabs(x);
...
//		SET_LOW_WORD(t1,0);
		val.value = t1;
		val.raw = ( val.raw ) & 0xffffffff00000000;
		t1 = val.value;
...
//		SET_HIGH_WORD(ax,ix);
		...
		tmp = ix;
		tmp = tmp << 32;
		tmp = tmp | ( val.raw & 0x00000000ffffffff );
		val.raw = tmp;
		ax = val.value;
...
//		SET_HIGH_WORD(t_h,((ix>>1)|0x20000000)+0x00080000+(k<<18));
		val.value = t_h;
		tmp = (( ix >> 1 ) | 0x20000000 ) + 0x00080000 + ( k << 18 );
		tmp = tmp << 32;
		tmp = tmp | ( val.raw & 0x00000000ffffffff );
		val.raw  = tmp;
		t_h = val.value;
...
//	GET_HIGH_WORD(j,z);
	...
	if (( j >> 20 ) <= 0 )
	{
		z = Self->math_d_scalbn(z,n); /* subnormal output */
	}
	else
	{
//		SET_HIGH_WORD(z,j);
